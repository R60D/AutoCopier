...
currentsizecount = 0
currentfilecount = 0
currentfilelocsource = ""
currentfilelocdest = ""
def callbacker(a, b, c):
    currentfilesize = (b/c)*currentsize
    print(f'{currentfilecount}/{filecount} [{round(currentsizecount+currentfilesize,2)}/{round(sizecount,2)}]MB {currentfilelocsource}->{currentfilelocdest}')
...
def getfilesize(file):
    ...
    number = file_stats.st_size / (1024 * 1024)
    return number


def copypath(source_folder,destination_folder):
    global sizecount
    global filecount
    global currentfilecount
    global currentsizecount
    global currentsize
    global currentfilelocsource
    global currentfilelocdest
    print("------------------------------------")
    ...
    if not(path.exists(destination_folder)):
        print(destination_folder+" does not exist")
        print("------------------------------------")
        return
    print(destination_folder+" exists")
    print(source_folder+" to "+destination_folder)
    print("------------------------------------")
    # fetch all files

    for root, subdirs, files in walk(source_folder):
        filecount += len(files)
        for file in files:
            sizecount += getfilesize(root+"\\"+file)
    for root, subdirs, files in walk(source_folder):
        for file in files:
            currentfilecount += 1
            currentsize = getfilesize(root+"\\"+file)
            source_file = root+"\\"+file #full path in dest
            currentfilelocsource = source_file
            
            dest_file = destination_folder+root[len(destination_folder):]+"\\"+file #full path in dest
            dest_path = destination_folder+root[len(destination_folder):]
            currentfilelocdest = dest_file
            try:
                copy_with_callback(source_file, dest_file,callbacker)
                currentsizecount += getfilesize(root+"\\"+file)
            except:
                makedirs(dest_path)

copypath(sys.argv[1],sys.argv[2])